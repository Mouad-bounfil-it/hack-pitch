...
/*Apr 25 - May 2, 2020 */
export function formatEventDate(start = null, end = null, isProgram = false) {
  if (isProgram) {
    if (start && end) {
  ...
  if (start && end) {
    return `${formatDateTimeNoYear(new Date(start))} - ${formatDateTimeNoYear(
      new Date(end)
    )}, ${format(new Date(start), "yyyy")}`;
  }

  if (start) {
    return formatDateTimeNoYear(start);
  }
